...
func NewArgs() Args {
	return Args{Style: Style, Heteronym: Heteronym, Separator: Separator, Fallback: Fallback, FallStr: []string{}}
}
...
func SinglePinyin(r rune, a *Args) []string {
	...
	if ok {
		...
		a.FallIsContinuous = true
		tempArr := a.Fallback(r, a)
		a.FallStr = append(a.FallStr, tempArr...)
	}
	if len(pys) > 0 {
		if !a.Heteronym {
			pys = []string{pys[0]}
		}
		if a.FallIsContinuous {
			arr := []string{strings.Join(a.FallStr, "")}
			pys = append(arr, pys...)
			a.FallStr = []string{}
			a.FallIsContinuous = false
		}
...
func Pinyin(s string, a *Args) [][]string {
	...
	if len(a.FallStr) > 0 {
		pys = append(pys, []string{strings.Join(a.FallStr, "")})
	}
	return pys
}
